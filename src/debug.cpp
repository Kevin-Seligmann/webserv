...
void showParse(const ParsedServers& config) {
    ...
    for (size_t i = 0; i < config.size(); ++i) {
        ...
        std::cout << YELLOW << "\n--- Server Block " << (i + 1) << " ---" << RESET << std::endl;
        
        // Listen directives
        std::cout << BLUE << "Listen Directives:" << RESET << std::endl;
        ...
        // Server names
        std::cout << BLUE << "Server Names:" << RESET << std::endl;
        ...
        // Root
        std::cout << BLUE << "Root: " << RESET << server.root << std::endl;
        
        // Index files
        std::cout << BLUE << "Index Files:" << RESET << std::endl;
        ...
        // Error pages
        std::cout << BLUE << "Error Pages:" << RESET << std::endl;
        ...
        if (server.locations.empty()) {
            ...
            for (std::map<std::string, ParsedLocations>::const_iterator it = server.locations.begin();
                 it != server.locations.end(); ++it) {
                const std::string& path = it->first;
                const ParsedLocations& loc = it->second;
    ...
    std::cout << GREEN << "\n=== END PARSED CONFIG DEBUG ===" << RESET << std::endl;
}
...
void showServers(ServersManager ws) {
    ...
    for (ws_it = ws.serversManager.begin(); ws_it != ws.serversManager.end(); ++ws_it) {
        ...
        for (s_it = servers_vec.begin(); s_it != servers_vec.end(); ++s_it, ++i) {
            ...
            std::cout << "  Listen: " << srv.getListens() << std::endl;
