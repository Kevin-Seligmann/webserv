...
void ResponseManager::write_file()
{
    ...
    if (_wr_file_it == _request.body.content.end())
    {
        ...
        _buffer.put_header("Server", "Webserv");
        _buffer.put_header_time("Date", time(NULL));
        _buffer.put_header_number("Content-Length", _request.body.content.size());
...
void ResponseManager::set_redirecting_location(const std::string& redir)
{
    _redirecting_location = redir;
}
