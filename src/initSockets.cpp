...
bool socketsInit(SocketsManager listenSockets, ServersManager webServers) {
    ...
    std::map<Listen, std::vector<Servers> >::iterator it = webServers.serversManager.begin();
    ...
    // crear estructura inversa dentro de listen sockets... que sea un mapa fd to Listen
        // con el fd se llega a Listen y con esto servirá para llegar a vector Servers
